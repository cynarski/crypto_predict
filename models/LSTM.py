...
class MyLSTM:
    ...
    def fit(self, data):
        scaled_data = self.sc_in.fit_transform(data)
        ...
        for epoch in range(self.epochs):
            history = self.model.fit(X_train, Y_train, epochs=1, validation_data=(X_val, Y_val), verbose=0)
            loss = history.history['loss'][0]
            # socketio.emit('log', {'message': f"LSTM Epoch {epoch + 1}/{self.epochs}: Loss = {loss:.4f}"})
            # socketio.emit('progress', {'progress': (epoch + 1) / self.epochs * 100})
